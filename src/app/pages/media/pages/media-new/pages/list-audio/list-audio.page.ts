...
export class ListAudioPage implements OnInit {
  ...
  ionViewWillEnter() {
    ...
    localStorage.setItem("media_type_new", "audio");
    this.checkSearch = false;
    this.checkkkk = true;
    localStorage.removeItem("media_type");
    localStorage.setItem("media_type", "audio");
    this.indexPage = 1;
    this.checkBackPage = true;
    this.loadData();
    this.globalBack = this.globalFooService
      .getObservableBackGetNewAudio()
      .subscribe((data) => {
        this.loadData();
      });
    if (localStorage.getItem("media_type") == "audio") {
  ...
  loadData() {
    ...
    if (localStorage.getItem("index_page_new")) {
      ...
      if (this.checkBackPage) {
        this.globalFooService.publishDataChangePageNewAudio({
          item: this.items,
          page: this.indexPage,
          check: true,
        });
  ...
  onChangePage(ev) {
    if (!this.checkSearchPage && !this.checkFilterAudio) {
      if (ev != 0 && !this.checkSearch) {
        ...
        if (localStorage.getItem("media_type_new") == "audio") {
