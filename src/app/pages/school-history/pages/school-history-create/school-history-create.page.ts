import { Component, OnInit, ɵConsole } from "@angular/core";
import {
  Validators,
  FormControl,
  FormGroup,
  FormBuilder,
} from "@angular/forms";
import { SchoolHistoryService } from "src/app/services/school-history.service";
import { Router } from "@angular/router";
import * as moment from "moment";
import { TranslateService } from "@ngx-translate/core";
import { DomSanitizer } from "@angular/platform-browser";

@Component({
  selector: "app-school-history-create",
  templateUrl: "./school-history-create.page.html",
  styleUrls: ["./school-history-create.page.scss"],
})
export class SchoolHistoryCreatePage implements OnInit {
  public isActiveArr = [
    { value: true, name: "Active" },
    { value: false, name: "Inactive" },
  ];
  public checkImage = false;
  public img = new Image();
  public checkError = false;
  public formData = new FormData();
  public publishDate;
  public checkQuill = false;
  public checkSubmit = false;
  public temp_photo;
  public title = "";
  public fileTypes = ["image/jpeg", "image/png"];
  public check_file_type = false;

  createHistoryForm: FormGroup = this.formBuilder.group({
    title: ["", [Validators.required]],
    push_date: ["", [Validators.required]],
    logo: ["", [Validators.required]],
    isActive: ["", [Validators.required]],
    // quill: ['', [Validators.required]],
  });

  public editor;
  public changed;
  image: any;
  constructor(
    private formBuilder: FormBuilder,
    private schoolHistoryService: SchoolHistoryService,
    private router: Router,
    private translate: TranslateService,
    private sanitizer: DomSanitizer
  ) {
    this.translate.use(localStorage.getItem("language"));
  }

  ngOnInit() {}

  ionViewWillEnter() {
    this.translate.use(localStorage.getItem("language"));
  }

  createHistory() {
    this.checkSubmit = true;
    // if(this.img.src == '' && this.check_file_type){
    //   this.checkImage = false
    // }

    if (
      this.createHistoryForm.valid &&
      this.checkQuill &&
      !this.check_file_type
    ) {
      let formData = new FormData();
      formData.append("title", this.createHistoryForm.controls.title.value);
      formData.append("content", this.changed);
      // this.formData.append('publish_date', this.createHistoryForm.controls.push_date.value)
      formData.append(
        "publish_date",
        moment(this.createHistoryForm.controls.push_date.value).format(
          "YYYY-MM-DD HH:mm"
        )
      );
      formData.append(
        "is_active",
        this.createHistoryForm.controls.isActive.value
      );
      formData.append("photo", this.temp_photo, this.temp_photo.name);
      // formData.append('created_by', "1")
      // formData.append('updated_by', "2")
      this.schoolHistoryService
        .createSchoolHistory(formData)
        .subscribe((data) => {
          this.router.navigate(["/school-history"]);
        });
    } else {
      this.checkError = true;
    }
  }

  onChange(element): void {
    this.check_file_type = false;
    let file = element[0];
    this.temp_photo = element[0];
    let fileReader: FileReader = new FileReader();
    if (!this.validFileType(element[0].type)) {
      this.check_file_type = true;
    } else {
      this.temp_photo = element[0];
      this.img.src = URL.createObjectURL(file);
    }
    // this.img.src = URL.createObjectURL(file);
    this.title = file.name;
    this.img.title = file.name;
    fileReader.readAsDataURL(file);
    this.checkImage = true;
  }

  changePublishDate() {
    this.publishDate = this.createHistoryForm.controls.push_date.value;
  }

  handleEditorCreated(event: any) {
    this.editor = event;
  }

  handleChange(event: any) {
    this.checkQuill = true;
    this.changed = event.html;
  }

  validFileType(file) {
    return this.fileTypes.includes(file);
  }

  zoomPicture(imgUrl) {
    this.image = this.sanitizer.bypassSecurityTrustResourceUrl(imgUrl);
  }

  closeZoom() {
    this.image = "";
  }
}
